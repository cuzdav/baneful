...
public:
  ...
  void add_level_type_override(char block,
  ...
  Color get_color(char block, RuleSide side) {
    return color::for_side(block_to_color_map_[block], side);
  }

  int add_vertex(std::string_view vertex, RuleSide side, Verticies & verticies) {
    char block = vertex[0];
    char transformed_block = block - block_fixup_map_[block];
...
private:
  ...
  Color get_or_create_color_by_name(std::string const & color_name) {
    auto
