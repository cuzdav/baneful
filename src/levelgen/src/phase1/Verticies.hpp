...
public:
  ...
  int
  name_index_of(std::string_view vertex, color::FinalColor final_color) const;
  int
  name_index_of_checked(std::string_view vertex, color::FinalColor final_color) const;

  std::string const &
  name_of(int index) const;
  ...
  int
  add_vertex_single(std::string_view vertex, block::FinalBlock block,
  ...
  // internal name
  static char
  color_to_char(color::FinalColor color);
  static color::FinalColor
  char_to_color(char color_char);

  static std::string
  internal_name(std::string_view vertex_id_string, color::FinalColor final_color);
